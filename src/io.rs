 ...
 */
pub fn load_todos() -> anyhow::Result<Todos> {
 ...
 * let todos: Todos = todo::io::load_todos_from_file("/tmp/test_todos.json").unwrap();
 * ```
 */
pub fn load_todos_from_file<P: AsRef<path::Path>>(file: P) -> anyhow::Result<Todos> {
    ...
    read_data(&mut file)
        .with_context(|| format!("failed to read todos from file: {}", path_ref.display()))
}
 ...
 * A `Result` containing the loaded todos, or an error if one occurred.
 */
fn read_data(file: &mut std::fs::File) -> Result<Todos, serde_json::Error> {
    let reader = BufReader::new(file);
    match serde_json::from_reader(reader) {
        Ok(data) => Ok(data),
        Err(e) if e.is_eof() => Ok(Todos::new()),
        Err(e) => Err(e.into()),
    }
}
 ...
 * ```
 * use todo::model::Todos;
 * let todos: Todos = Todos::new();
 * todo::io::save_todos(&todos).unwrap();
 * ```
 */
pub fn save_todos(todos: &Todos) -> anyhow::Result<()> {
    let home = env::var("HOME") // Unix
        .or_else(|_| env::var("USERPROFILE")) // Windows
        .expect("HOME not set");
 ...
 * ```
 */
pub fn save_todos_to_file<P: AsRef<path::Path>>(todos: &Todos, file: P) -> anyhow::Result<()> {
    let path_ref = file.as_ref();
    let mut file = OpenOptions::new()
        .write(true) // Enable writing
        .create(true) // Create if doesn't exist
        ...
        .open(path_ref)
        .with_context(|| format!("failed to open file: {}", path_ref.display()))?;
    write_data(todos, &mut file)
        .with_context(|| format!("failed to save todos to file: {}", path_ref.display()))
}
 ...
 * A `Result` indicating success or failure.
 */
fn write_data(todos: &Todos, file: &mut std::fs::File) -> Result<(), serde_json::Error> {
